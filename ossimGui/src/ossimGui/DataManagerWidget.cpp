...
void ossimGui::DataManagerInputConnectionFolder::populateChildren()
{
   ...
   if((inputList.size() == childCount())&&!inputList.empty())
   {
...
void ossimGui::DataManagerImageChainItem::dropItems(QList<DataManagerImageChainItem*>& chainItemList, 
   ...
   if(m_jobQueue.valid())
   {
      ...
      if (fileNames.size() > 0) 
      {
         for (int i = 0; i < fileNames.size(); ++i)
         {
            ...
            job->setName(ossimString("open file://") + fileNames.at(i).toAscii().data());
      ...
      while(iter != result.end())
      {
         ...
         if(node)
         {
            ...
            if(diplayObj&&diplayObj->display())
            {
               ...
               {
                  ossimDrect rect = subWindow->scrollWidget()->viewportBoundsInViewSpace();
                  
                  QRect scrollRect = subWindow->scrollWidget()->viewToScroll().mapRect(QRect(rect.ul().x, rect.ul().y, rect.width(), rect.height()));
                  
                  ossimGui::Image image(scrollRect.size(), QImage::Format_RGB32);
                  image.setOffset(QPoint(scrollRect.x(), scrollRect.y()));
                  ossimGui::ImageScrollWidget::Layer* layer = subWindow->scrollWidget()->layers()->layer((ossim_uint32)0);
