...
class OSSIMGUI_DLL ossimSetViewVisitor : public ossimTypeNameVisitor
{
public:
   ossimSetViewVisitor(ossimObject* view,
   ...
   ossimSetViewVisitor(const ossimSetViewVisitor& src)
   :ossimTypeNameVisitor(src),
   m_obj(src.m_obj)
   {
      m_viewPoint.makeNan();
   }
   virtual ossimRefPtr<ossimVisitor> dup()const{return new ossimSetViewVisitor(*this);}
...
:m_widget(0)
{
   ...
   m_currentViewPoint.makeNan();
   m_viewManip = new ViewManipulator();
}
...
void ossimGui::ImageActions::fitToWindow()
{
   ...
   QSize size = m_widget->viewport()->size();
   m_viewManip->fit(currentViewSize, ossimDrect(0,0,size.width()-1, size.height()-1));
   
   ossimSetViewVisitor viewVisitor(m_viewManip->getObjectAs<ossimObject>());
   m_widget->connectableObject()->accept(viewVisitor);
   viewVisitor.setView();
   
   m_currentViewPoint = m_widget->viewportBoundsInViewSpace().midPoint();
   setupAndExecuteSyncing();
}

void ossimGui::ImageActions::fullRes()
{
   ossimDrect currentBounds = m_widget->viewportBoundsInViewSpace();
   ossimDpt center = currentBounds.midPoint();
   m_viewManip->fullRes(center);
...
void ossimGui::ImageActions::zoomIn(double factor)
{
   ...
   ossimDpt center = currentBounds.midPoint();
   m_viewManip->zoomIn(center, factor);
...
void ossimGui::ImageActions::zoomOut(double factor)
{
   ...
   ossimDrect currentBounds = m_widget->viewportBoundsInViewSpace();
   ossimDpt center = currentBounds.midPoint();
   m_viewManip->zoomOut(center, factor);
...
void ossimGui::ImageActions::mouseMove(QMouseEvent* event, 
                                       const ossimDrect& viewportRectInViewSpace, const ossimDpt& viewPoint)
{
  ...
  // std::cout << "mouseMove: " << localPoint << std::endl;
}

void ossimGui::ImageActions::mousePress(QMouseEvent* event, const ossimDrect& viewportRectInViewSpace, const ossimDpt& viewPoint)
{
   ...
   //std::cout << "mousePress: " << localPoint << std::endl;
}
void ossimGui::ImageActions::mouseRelease(QMouseEvent* event, const ossimDrect& viewportRectInViewSpace, const ossimDpt& viewPoint)
{
   if(event->button()&Qt::LeftButton)
   {
      m_leftButtonPressed = false;
   ...
   //   std::cout << "mouseRelease: " << localPoint << std::endl;
}

void ossimGui::ImageActions::mouseDoubleClick(QMouseEvent* event, const ossimDrect& viewportRectInViewSpace, const ossimDpt& viewPoint)
{
  // std::cout << "mouseDoubleClick: " << localPoint << std::endl;
}

void ossimGui::ImageActions::wheel(QWheelEvent* event,  const ossimDrect& viewportRectInViewSpace, const ossimDpt& viewPoint)
{
   event->accept();
   ...
   switch(event->modifiers())
   {
      ...
      default:
      {
         ossimRefPtr<ossimRefreshEvent> refreshEvent = new ossimRefreshEvent(ossimRefreshEvent::REFRESH_POSITION);
         ossimEventVisitor visitor(refreshEvent.get());
         
         ossimIpt pt(viewportRectInViewSpace.midPoint());
         ...
         refreshEvent->setPosition(pt);
         m_widget->connectableObject()->accept(visitor);
         break;
      }
   }
...
void ossimGui::ImageActions::addActions(QMainWindow* mainWindow)
{
   ...
   if(!action)
   {
      action = imageMenu->addAction("Select Bands");
   ...
   if(!m_visitor.m_viewInterfaces.empty())
   {
      ossimViewInterface* viewInterface = dynamic_cast<ossimViewInterface*> (m_visitor.m_viewInterfaces[0].get());
      ossimConnectableObject* connectable = dynamic_cast<ossimConnectableObject*> (m_visitor.m_viewInterfaces[0].get());
      
      if(connectable&&viewInterface)
      {
         ossimImageSource* input = dynamic_cast<ossimImageSource*> (connectable);
         ossimImageSource* inputFullResGeometry = input;
         if(connectable->getInput())
         {
            inputFullResGeometry = dynamic_cast<ossimImageSource*>(input->getInput());
         }
         if(inputFullResGeometry)
         {
            ossimRefPtr<ossimImageGeometry> geom = inputFullResGeometry->getImageGeometry();
            if(geom.valid()&&geom->getProjection())
            {
               m_viewManip->setFullResScale(geom->getProjection()->getMetersPerPixel());
            }
         }
         ossimDrect currentViewSize = m_widget->connectableObject()->getBounds();
         QSize size = m_widget->viewport()->size();
         
         ossimObject* view = viewInterface->getView();
         if(view)
         {
            m_viewManip->setObject(view->dup());
         }
      }
   }
}
...
void ossimGui::ImageActions::setupAndExecuteSyncing()
{
   ...
   if(geom.valid()&&!m_currentViewPoint.hasNans())
   {
      int syncType = m_syncType;
      View view(syncType, m_currentViewPoint, geom.get());
      if(!m_leftButtonPressed) view.setSyncType(View::SYNC_TYPE_CURSOR, false);
...
:MdiSubWindowBase(parent, flags)
{
   ...
   m_imageScrollWidget = new ImageScrollWidget();
   m_imageScrollWidget->setConnectableObject(static_cast<ConnectableImageObject*>(m_connectableObject.get()));
   setWidget(m_imageScrollWidget);
   m_actions->setWidget(m_imageScrollWidget);
...
ossimGui::ImageMdiSubWindow::~ImageMdiSubWindow()
{
   ...
   if(m_connectableObject.valid())
   {
      ...
      m_imageScrollWidget->setConnectableObject(0);
   }
   if(m_actions)
   {
      delete m_actions;
      ...
      while(currentWindow != wList.end())
      {
         ...
         else if(w == this)
         {
            ...
            if(geom.valid())
            {
               ...
               ossimDrect viewSpaceRect = m_imageScrollWidget->viewportBoundsInViewSpace();
               ossimDpt viewPoint = viewInfo.lookPosition();
               ossimDpt localPt;
               m_imageScrollWidget->viewToLocal().map(viewPoint.x, viewPoint.y, &localPt.x, &localPt.y);
...
{
   m_imageScrollWidget->setJobQueue(q);
}

ossimGui::ImageScrollWidget* ossimGui::ImageMdiSubWindow::scrollWidget()
{
   return m_imageScrollWidget;
}
...
{ 
   ...
   oldCenter.makeNan();
   ossimDrect rect = m_imageScrollWidget->viewportBoundsInViewSpace();
   
   ossimRefPtr<ViewManipulator> viewManip = m_actions->viewManipulator();
   ossimRefPtr<ossimImageGeometry> widgetGeometry = m_imageScrollWidget->getGeometry();
   if(viewManip.valid()&&widgetGeometry.valid())
   {
   ...
   ossimSetViewVisitor visitor(geom.get());
   ...
   if(trackGeom.valid()&&hasProjection)
   {
      ...
      if(!referencePosition.isLatNan()&&!referencePosition.isLonNan())
      {
         ...
         {
            m_imageScrollWidget->setTrackPoint(viewPoint);
         }
         else
         {
            ossimDpt nanPt;
            nanPt.makeNan();
            m_imageScrollWidget->setTrackPoint(nanPt);
         }
         if(syncType&View::SYNC_TYPE_POSITION||
            syncType&View::SYNC_TYPE_GEOM)
         {
            m_imageScrollWidget->setPositionGivenView(viewPoint);
         }
      }
      else 
      {
   ...
   if(viewManip.valid())
   {
      ...
      {
         viewManip->setObject(view.geometry()->dup());
      }
   }
}
...
{
   return m_imageScrollWidget->getGeometry();
}
