...
//
//*******************************************************************
//  $Id: ossimGdalTileSource.cpp 21634 2012-09-06 18:15:26Z dburken $
      ...
      for(ossim_int32 band = 0; band < (ossim_int32)bands; ++band)
      {
	   ...
	   if(hasMetaData())
	   ...
	   else 
           {
              ...
              {
//		   theNullPixValues[band] = ossimGdalType().getNan(getOutputScalarType());
                 theNullPixValues[band] = ossim::defaultNull(getOutputScalarType());
              }
           }
           if(!minOk||!maxOk)
           {
//               GDALComputeRasterMinMax(aBand, TRUE, adfMinMax);
               
//                theMinPixValues[band] = adfMinMax[0];
//                theMaxPixValues[band] = adfMinMax[1];
               theMinPixValues[band] = ossim::defaultMin(getOutputScalarType());
               theMaxPixValues[band] = ossim::defaultMax(getOutputScalarType());
         ...
         else
         {
            theMinPixValues[band] = ossim::defaultMin(getOutputScalarType());
            theMaxPixValues[band] = ossim::defaultMax(getOutputScalarType());
//            theMinPixValues[band] = ossimGdalType().getNan(getOutputScalarType());
//            theMaxPixValues[band] = ossimGdalType().getNan(getOutputScalarType());
         }
      }
   }
}
...
ossimString ossimGdalTileSource::className()const
{
...
/**/
bool ossimGdalTileSource::getGeoTransform(double*  geoTransform)
{
    bool transOk = GDALGetGeoTransform( theDataset, geoTransform ) == CE_None;
    return transOk;
}
...
std::string ossimGdalTileSource::getProjectionInfo(ossimMapProjection* mapProj){
    if (mapProj)
    {
        ossimOgcWktTranslator translator;
