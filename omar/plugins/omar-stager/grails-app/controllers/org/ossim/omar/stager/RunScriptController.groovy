...
class RunScriptController implements ApplicationContextAware{
    ...
    def stop()
    {
        ...
        {
           // println quartzScheduler.unscheduleJob(params.name, params.group)
            println quartzScheduler.interrupt(params.name, params.group)
        }
        else
        {
        }
    ...
    def jobs()
    {
        def jobs = "${runScriptService.listScheduledJobs() as JSON}"//"${runScriptService.listJobTriggersByGroup("STAGER_SCRIPTS") as JSON}"
        response.contentType = "application/json"
        response.outputStream << jobs
