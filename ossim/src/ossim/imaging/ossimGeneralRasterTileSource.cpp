...
//*******************************************************************
//  $Id: ossimGeneralRasterTileSource.cpp 21631 2012-09-06 18:10:55Z dburken $
...
bool ossimGeneralRasterTileSource::setOutputBandList(const std::vector<ossim_uint32>& band_list)
{
   ...
   if ( isBandSelector() )
   {
      result = ossimImageHandler::setOutputBandList( band_list, m_outputBandList );
...
void ossimGeneralRasterTileSource::allocateBuffer( const ossimImageData* tile )
{
   ...
   if ( tile )
   {
      ...
      if ( isBandSelector() && ( m_rasterInfo.interleaveType() == OSSIM_BIP ) )
      {
